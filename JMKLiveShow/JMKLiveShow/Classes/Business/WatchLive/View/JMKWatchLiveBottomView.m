...
#import "JMKLikeObject.h"
#import "JMKKeyboardToolView.h"


@interface JMKWatchLiveBottomView ()<JMKKeyboardToolViewDelegate>
...
/// 聊天工具栏
@property (nonatomic, strong) JMKKeyboardToolView *messageToolView;
/// 记录是否被禁言
...
/// 点赞按钮
@property (nonatomic, strong) JMKLikeObject *likeBtn;
/// 是否是聊天
...
@implementation JMKWatchLiveBottomView
...
- (void)initWithData
{
    [self.headImg sd_setImageWithURL:[NSURL URLWithString:[VHallApi currentUserHeadUrl]] placeholderImage:[UIImage imageNamed:@"vh_no_head_icon"]];
...
- (void)permissionsCheckWithWebinarId
{
    ...
    [JMKWebinarBaseInfo permissionsCheckWithWebinarId:self.webinarInfoData.webinar.data_id webinar_user_id:self.webinarInfoData.webinar.userinfo.user_id scene_id:@"1" success:^(VHPermissionConfigItem * _Nonnull item) {
            
        VHLog(@"permissionsCheckWithWebinarId -- ");
...
- (JMKLikeObject *)likeBtn
{
    if (!_likeBtn) {
        _likeBtn = [[JMKLikeObject alloc] init];
...
- (JMKKeyboardToolView *)messageToolView
{
    if (!_messageToolView) {
        _messageToolView = [[JMKKeyboardToolView alloc] init];
        [_messageToolView setHidden:NO];
        [_messageToolView setDelegate:self];
        [[JMKUITool mainWindow] addSubview:_messageToolView];
    }
