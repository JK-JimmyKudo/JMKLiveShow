...
@interface JMKWatchVideoView ()<VHallMoviePlayerDelegate, VHWebinarInfoDelegate, UIGestureRecognizerDelegate>
...
/// 弹幕
@property (nonatomic, strong) JMKDanmu *danmu;
...
- (void)updataToWebinarInfo:(VHWebinarInfo *)webinarInfo
{
    ...
    NSString *nickName = [JMKUITool isBlankString:webinarInfo.author_nickname] ? @"主持人" : webinarInfo.author_nickname;
    ...
    _nickNameLab.text = [JMKUITool substringToIndex:kMaxNicknameLength text:nickName isReplenish:YES];
...
- (void)startPlay
{
    ...
    VHallChatModel * msgModel = [[VHallChatModel alloc]init];
...
{
    [JMKProgressHud showToast:@"直播已结束"];
...
{
    [JMKProgressHud showToast:isKickout ? @"被踢出" : @"取消踢出"];
...
{
    [JMKProgressHud showToast:@"主持同意上麦"];
...
- (void)rateBtnAction
{
    ...
    for (int i = 0; i < rateAry.count; i++) {
        ...
        UIAlertAction *alertAction = [UIAlertAction actionWithTitle:rateStr
            ...
            [JMKProgressHud showToast:[NSString stringWithFormat:@"已切换为%@x倍速", rateStr]];
    ...
    [[JMKUITool viewControllerWithView:self] presentViewController:alertController animated:YES completion:nil];
}
...
- (UIImageView *)headImg
{
    if (!_headImg) {
        ...
        [_headImg sd_setImageWithURL: [NSURL URLWithString:@"http://gips2.baidu.com/it/u=195724436,3554684702&fm=3028&app=3028&f=JPEG&fmt=auto?w=1280&h=960"]];
    }
