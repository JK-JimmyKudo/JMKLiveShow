...
#import "JMKChatTableViewCell.h"
#import "JMKChatGiftCell.h"
#import "JMKChatLotteryCell.h"
#import "JMKChatPushScreenCardCell.h"


@interface JMKChatView ()<VHallChatDelegate, UITableViewDataSource, UITableViewDelegate,JXCategoryListContentViewDelegate>
/// 聊天
...
- (void)loadHistoryWithPage:(NSInteger)page
{
    ...
    for (NSInteger i = 0; i < 10; i++) {
        ...
        customModel.roleName = 1;
        customModel.content = @"content";
        customModel.info = [NSMutableDictionary dictionary];
        [self.chatDataSource addObject:customModel];
    }
    
    
    for (NSInteger i = 0; i < 4; i++) {
        VHallChatModel *customModel = [VHallChatModel new];
        customModel.text = @"nickName";
        customModel.atList = @[@"nihao",@"wowoso"];
        customModel.type = @"content";
        customModel.imageUrls = @[@"https://i1.hdslb.com/bfs/archive/08457ffe0f64003c3d166257fe8387d1cf1acb9e.jpg",@"https://i1.hdslb.com/bfs/archive/08457ffe0f64003c3d166257fe8387d1cf1acb9e.jpg"];
        [self.chatDataSource addObject:customModel];
    }
...
{
    [JMKProgressHud showToast:forbidChat ? @"您以被禁言" : @"您已被取消禁言"];
...
{
    [JMKProgressHud showToast:allForbidChat ? @"开启全体禁言" : @"取消全体禁言"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.chatDataSource.count > indexPath.row) {
        ...
        JMKChatTableViewCell *cell = [JMKChatTableViewCell createCellWithTableView:tableView];

        JMKChatCustomCell *customCell = [JMKChatCustomCell createCellWithTableView:tableView];
        __weak __typeof(self) weakSelf = self;
...
//        JMKChatGiftCell *giftCell = [JMKChatGiftCell createCellWithTableView:tableView];
//
//        JMKChatLotteryCell *lotteryCell = [JMKChatLotteryCell createCellWithTableView:tableView];
//        
...
//        };
//        
//        JMKChatPushScreenCardCell *pushScreenCardCell = [JMKChatPushScreenCardCell createCellWithTableView:tableView];
...
- (void)sendText:(NSString *)text
{
    if (_chat.isAllSpeakBlocked) {
        [JMKProgressHud showToast:@"已开启全体禁言"];
        return;
    }

    if (_chat.isSpeakBlocked) {
        [JMKProgressHud showToast:@"您已被禁言"];
        return;
    }

    if (text.length == 0) {
        [JMKProgressHud showToast:@"发送的消息不能为空"];
        return;
    }
...
- (UITableView *)chatTableView
{
    if (!_chatTableView) {
        ...
        _chatTableView.backgroundColor = [UIColor blueColor];
