...
- (UIButton *)sendBtn
{
    if (!_sendBtn) {
        ...
        _sendBtn.accessibilityLabel = VHTests_Gift_SendGift;
...
@interface JMKFashionStyleGiftListView ()<UICollectionViewDelegate, UICollectionViewDataSource, UIGestureRecognizerDelegate>
...
@implementation JMKFashionStyleGiftListView
...
- (void)layoutSubviews
{
    ...
    [JMKUITool clipView:_contentView corner:UIRectCornerTopLeft | UIRectCornerTopRight anSize:CGSizeMake(8, 8)];
...
- (void)requestWebinarUsingGiftListWithRoomId
{
    ...
    for (NSInteger i = 0; i < 10; i ++ ) {
        VHallGiftListItem *item = [[VHallGiftListItem alloc]init];
        
        item.giftId = @"111";                           ///<礼物id
        item.image_url = @"https://bpic.588ku.com/element_origin_min_pic/23/06/09/9f3d42d77da2f7fe15ead988db5e7be5.jpg";                     ///<礼物图片
        item.name = @"火箭";                          ///<礼物名称
        item.price = @"999";                           ///<礼物价格
        item.source_status = 1;                   ///<来源类型：0 web 1 app
        item.source_type = 0;                    ///<0 系统礼物 1 自定义礼物
        [giftList addObject:item];
    }
...
//    [VHallGiftObject webinarUsingGiftListWithRoomId:self.webinarInfoData.interact.room_id
//                                           complete:^(NSArray<VHallGiftListItem *> *_Nonnull giftList, NSError *_Nonnull error) {
//        if (giftList) {
//            weakSelf.dataSource = [NSMutableArray arrayWithArray:giftList];
//            [weakSelf.collectionView reloadData];
//        }
//
//        if (error) {
//            [JMKProgressHud showToast:error.localizedDescription];
//        }
//    }];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.clickSendBtnBlock = ^{
        ...
        [VHallGiftObject sendGiftWithRoomId:weakSelf.webinarInfoData.interact.room_id
                                    channel:@"WEIXIN"
                               service_code:@"QR_PAY"
                                   giftItem:giftItem
                                   complete:^(VHallSendGiftModel *_Nonnull sendGiftModel, NSError *_Nonnull error) {
            if (sendGiftModel) {
            }
...
- (UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        _collectionView.accessibilityIdentifier = VHTests_Gift_CollectionId;
