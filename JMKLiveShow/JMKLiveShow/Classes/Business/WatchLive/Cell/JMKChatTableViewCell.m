...
@interface JMKChatTableViewCell ()<UICollectionViewDelegate, UICollectionViewDataSource>
...
@implementation JMKChatTableViewCell


+ (JMKChatTableViewCell *)createCellWithTableView:(UITableView *)tableView
{
    JMKChatTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"JMKChatTableViewCell"];

    if (!cell) {
        cell = [[JMKChatTableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"JMKChatTableViewCell"];
...
- (void)setModel:(VHallChatModel *)model
{
    ...
    // 昵称
    self.nickNameLab.text = [JMKUITool substringToIndex:8 text:model.user_name isReplenish:YES];
    // 时间
    self.timeLab.text = [JMKUITool substringFromIndex:11 text:model.time isReplenish:NO];
    // 身份
...
{
    self.replyNickNameLab.text = [JMKUITool substringToIndex:8 text:model.replyMsg.user_name isReplenish:YES];
...
{
    // 聊天内容
    NSString *content = [NSString stringWithFormat:@"%@%@", model.replyMsg ? @"回复 " : @"", [JMKUITool isBlankString:model.text] ? @"" : model.text];
...
//    [browser showFromVC:[JMKUITool viewControllerWithView:self]];
//
...
- (UILabel *)nickNameLab {
    if (!_nickNameLab) {
        ...
        _nickNameLab.textColor = JMKBlack45;
