...
@implementation JMKChatCustomCell


+ (JMKChatCustomCell *)createCellWithTableView:(UITableView *)tableView
{
    JMKChatCustomCell *cell = [tableView dequeueReusableCellWithIdentifier:@"JMKChatCustomCell"];

    if (!cell) {
        cell = [[JMKChatCustomCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"JMKChatCustomCell"];
...
- (void)setChatCustomModel:(VHChatCustomModel *)chatCustomModel
{
    ...
    /// 昵称
    chatCustomModel.nickName = @"ni好啊";
    /// 身份
    chatCustomModel.roleName = 1;
    /// 详情
    chatCustomModel.content = @" 出差 还常常被v v女孩v好v";
    /// 附加信息
    chatCustomModel.info =  @{};
    ...
    chatCustomModel.nickName = [JMKUITool substringToIndex:8 text:chatCustomModel.nickName isReplenish:YES];
