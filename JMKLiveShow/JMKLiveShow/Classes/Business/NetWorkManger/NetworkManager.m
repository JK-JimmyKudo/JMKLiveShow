...
#pragma mark - 开始获取缓存
-(void)startCacheRequest:(NetworkBaseRequest *)request{
    // 请求实体
    NSString *requestUrl = request.requestUrl;
    NSString *baseUrl = request.baseUrl;
    NSString *cacheURLStr = [NSString stringWithFormat:@"%@%@",baseUrl,requestUrl];
    BOOL is_cache = [NetworkCacheUtil checkCacheWithURL:cacheURLStr];
    if (is_cache) {
        NSDictionary *  responseObject = [NetworkCacheUtil  cacheJsonWithURL:cacheURLStr];
        [self _cacheRequestFinished:request responseObject:responseObject];
    }
}


- (void)_cacheRequestFinished:(NetworkBaseRequest *)request responseObject:(NSDictionary *)responseObject{
...
///数据缓存
-(void)_cache:(NetworkBaseRequest *)request task:(NSURLSessionTask *)task responseObject:(id _Nullable)responseObject{
    NSString *requestUrl = request.requestUrl;
    NSString *baseUrl = request.baseUrl;
    NSString *cacheURLStr = [NSString stringWithFormat:@"%@%@",baseUrl,requestUrl];
    NSDictionary *cacheDict = (NSDictionary *)responseObject;
    BOOL is_cache =  [NetworkCacheUtil saveJsonResponseToCacheFile:cacheDict andURL:cacheURLStr params:nil];
//    KLog(@"是否保存成功 is_cache -- %d",is_cache);
}
...
//    if (statusCode == NetworkResponseStatusCodeSuccess) {
//        ///缓存
//        [self _cache:request task:request.requestTask responseObject:dict];
//        id responseData = data;
//        if (data) {
//            Class class = [self _baseResponseClassOfBaseRequest:request];
//            if ([data isKindOfClass:NSDictionary.class]) {
